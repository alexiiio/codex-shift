...
- **启动周额度窗口：** 提前启动尚未开始计时的滚动周额度周期，使下一次重置时间相应提前，而不是等到未来真正使用 Codex 时才开始；仅对用户确认的账号发送一次刻意精简的请求。
...
在已经安装并登录 Codex CLI 的前提下，保存当前账号。`personal` 只是 profile 名称示例，请替换成你需要的名称。
...
这样可以让选中账号的滚动周额度周期立即开始计时，而不是等到下一次真正执行编程任务时才启动，从而使下一次重置时间相应提前。Codex Shift 会检测符合条件的账号，展示本次选用的模型与其支持的最低推理强度，支持逐账号选择，并在为每个选中账号发送一次刻意精简的请求前进行最终确认。选择取消不会发送请求，也不会消耗额度。
...
账号列表可以显示邮箱、订阅类型、周额度剩余量、重置时间、可用的已赚取重置次数和数据来源。`RESETS` 表示当前可以在 Codex 中兑换的 usage limit reset 次数；`0` 表示当前没有可用次数，`-` 表示已安装的 Codex 版本或账号响应未提供该信息。交互列表下方会通过 `Selected account` 明确指出 reset 详情属于哪个选中账号，并按本机时区显示其最近过期时间；距离过期不足 48 小时时，该条信息会显示为黄色。如果后端只返回部分明细，Codex Shift 会标记为 `Nearest known expiry`；如果完全没有提供明细，则仍显示准确的可用次数，并将过期信息标记为不可用。查看列表只会读取这些数值，不会兑换重置次数。`LIVE` 表示本次实时刷新成功，`CACHED` 表示实时查询失败并正在显示缓存，`UNAVAILABLE` 表示实时和缓存数据都不可用。重置时间使用运行 Codex Shift 的本机时区。如果尚未使用的周额度窗口返回了一个随查询时间向后移动的重置时间，Codex Shift 会显示 `Not started`，而不是显示这个动态时间。
...
部分账号只有在发出第一条 Codex 请求后才会开始计算周额度窗口。提前启动这个滚动周期，也会让下一次重置时间相应提前，而不是把周期开始和重置都推迟到未来真正使用 Codex 时。它不会增加额度总量，作用是调整当前周额度周期开始和重置的时间。运行：
