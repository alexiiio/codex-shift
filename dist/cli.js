...
function requireName(args, command) {
    if (args.length !== 1)
        throw new Error(`Usage: codex-shift ${command} <name>`);
    return args[0];
}
function requireNoArgs(args, command) {
    if (args.length !== 0)
        throw new Error(`Usage: codex-shift ${command}`);
}
function parseRemoveArgs(args) {
    if (args.length === 1)
        return { name: args[0], confirmed: false };
    if (args.length === 2 && args[1] === '--yes')
        return { name: args[0], confirmed: true };
...
function profileSource(profile, text, color) {
    if (profile.dataSource === 'live')
        return paint(color, ANSI.green, text);
    if (profile.dataSource === 'cached')
        return paint(color, ANSI.yellow, text);
...
function formatModel(plan) {
    return plan.model ?? 'Codex default';
}
function accountCount(count) {
    return `${count} ${count === 1 ? 'account' : 'accounts'}`;
}
function formatWeeklyPlanTable(plans, checked, selectedIndex, color = false) {
...
async function selectWeeklyInitializations(plans) {
    ...
    const render = () => {
        ...
        if (mode === 'select') {
            body.push('', paint(true, ANSI.dim, `${selectedPlans.length} of ${plans.length} accounts selected · Model choices are frozen for this run.`), '', formatHelp([['↑/↓', 'Move'], ['Space', 'Toggle'], ['A', 'All/none'], ['Enter', 'Continue'], ['Q', 'Quit']]));
        }
        else {
    ...
    return await new Promise((resolve) => {
        ...
        const onKeypress = (_value, key) => {
            ...
            if (key.name === 'escape') {
                ...
                if (checked.size === plans.length)
                    checked.clear();
                else
                    plans.forEach((plan) => checked.add(plan.name));
            }
...
async function initializeUnusedWeeklyWindows(args) {
    const dryRun = args.length === 1 && args[0] === '--dry-run';
    if (args.length > (dryRun ? 1 : 0))
        throw new Error('Usage: codex-shift init-week [--dry-run]');
    ...
    if (inspection.targets.length === 0) {
        ...
        if (inspection.unknown.length === 0) {
            ...
            console.log(paint(color, ANSI.dim, `  Checked ${accountCount(inspection.profiles.length)} · Everything is ready. No action needed.`));
        }
        else {
            const activeCount = inspection.profiles.length - inspection.unknown.length;
            console.log(paint(color, `${ANSI.bold}${ANSI.yellow}`, '⚠ No unstarted weekly usage windows were found.'));
            console.log(paint(color, ANSI.dim, `  ${accountCount(activeCount)} ${activeCount === 1 ? 'is' : 'are'} active · `
        ...
        case 'list':
            ...
            console.log('Refreshing account information...');
            {
                ...
                if (profiles.length > 1 && canUseInteractiveList())
                    profiles = await showInteractiveList(profiles);
                else
