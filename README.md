...
The account table can show the email, plan, weekly usage remaining, reset time, available earned resets, and data source. `RESETS` is the number of usage-limit reset credits currently available to redeem in Codex; `0` means none are available, while `-` means the installed Codex version or account response did not provide this information. Below the interactive table, `Selected account` explicitly identifies which profile the reset-credit details belong to and shows its next expiry in the machine's local timezone. Expiry within 48 hours is highlighted in yellow. If the backend returns only part of the credit details, Codex Shift labels the value `Nearest known expiry`; if no details are provided, the available count remains visible while expiry is marked unavailable. Listing accounts only reads these values and never redeems a reset. `LIVE` means the current refresh succeeded, `CACHED` means the live lookup failed and saved metadata is being shown, and `UNAVAILABLE` means neither live nor cached data is available. Reset times use the local timezone of the machine running Codex Shift. When an unused weekly window returns a reset time that advances with each lookup, Codex Shift displays `Not started` instead of the moving time.
